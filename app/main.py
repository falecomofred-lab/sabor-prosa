...
<body>
    <div class="login-container">
        ...
        <div class="card">
            ...
            <form onsubmit="return fazerLogin(event)">
                <div class="form-group"><label>Usuário</label><input type="text" id="usuario" placeholder="Digite seu usuário" required></div>
    ...
    <script>
        async function fazerLogin(e) {
            ...
            const resp = await fetch('/api/login', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({usuario,senha})});
            if (resp.ok) { const data = await resp.json(); localStorage.setItem('token',data.token); localStorage.setItem('usuario',data.usuario); window.location.href='/dashboard'; }
...
async def dashboard_page(request: Request):
    ...
    db.close()
    return render_template("dashboard.html", {"request":request,"total_produtos":len(pl),"total_estoque":sum(p.estoque_atual for p in pl),"margem_media":m,"alertas":len([p for p in pl if p.estoque_atual <= p.estoque_minimo])})
...
async def listar_produtos_html(request: Request):
    ...
    return render_template("produtos.html", {"request":request,"produtos":pl,"editando":None})
...
async def editar_produto_html(request: Request, pid: int):
    ...
    return render_template("produtos.html", {"request":request,"produtos":pl,"editando":p})
...
async def fornecedores_html(request: Request):
    ...
    return render_template("fornecedores.html", {"request":request,"fornecedores":fl,"editando":None})
...
async def editar_fornecedor_html(request: Request, fid: int):
    ...
    return render_template("fornecedores.html", {"request":request,"fornecedores":fl,"editando":f})
...
async def clientes_html(request: Request):
    ...
    return render_template("clientes.html", {"request":request,"clientes":cl})
...
async def eventos_html(request: Request):
    ...
    return render_template("eventos.html", {"request":request,"eventos":el})
...
async def conteudo_html(request: Request):
    db = SessionLocal(); pl = db.query(Produto).order_by(Produto.nome).all(); db.close()
    return render_template("conteudo.html", {"request":request,"produtos":pl})

@app.get("/pdv", response_class=HTMLResponse)
async def pdv_html(request: Request): return render_template("pdv.html", {"request":request})
...
async def kits_html(request: Request):
    ...
    return render_template("kits.html", {"request":request,"kits":kl,"produtos":pl,"editando":None})
...
async def editar_kit_html(request: Request, kid: int):
    ...
    return render_template("kits.html", {"request":request,"kits":kl,"produtos":pl,"editando":k})

@app.get("/busca", response_class=HTMLResponse)
async def busca_html(request: Request): return render_template("busca.html", {"request":request})

@app.get("/tela-cliente", response_class=HTMLResponse)
async def tela_cliente(request: Request): return render_template("tela_cliente.html", {"request":request})
...
async def vitrine_publica(request: Request): return render_template("vitrine.html", {"request": request})

@app.get("/gatilhos", response_class=HTMLResponse)
async def gatilhos_html(request: Request): return render_template("gatilhos.html", {"request": request})

@app.get("/estudio", response_class=HTMLResponse)
async def estudio_html(request: Request):
    db = SessionLocal(); pl = db.query(Produto).order_by(Produto.nome).all(); db.close()
    return render_template("estudio.html", {"request": request, "produtos": pl})

@app.get("/radar", response_class=HTMLResponse)
async def radar_html(request: Request): return render_template("radar.html", {"request": request})

@app.get("/rotas", response_class=HTMLResponse)
async def rotas_html(request: Request): return render_template("rotas.html", {"request": request})

@app.get("/monitor", response_class=HTMLResponse)
async def monitor_html(request: Request): return render_template("monitor.html", {"request": request})

@app.get("/checklist", response_class=HTMLResponse)
async def checklist_html(request: Request): return render_template("checklist.html", {"request": request})

@app.get("/tutorial", response_class=HTMLResponse)
async def tutorial_html(request: Request): return render_template("tutorial.html", {"request": request})

@app.get("/cadastro-evento", response_class=HTMLResponse)
async def cadastro_evento_page(request: Request): return render_template("cadastro-evento.html", {"request": request})
...
async def cadastro_evento_api(data: dict):
    ...
    origem = data.get("origem", "feira"); evento = data.get("evento", "QR Code")
...
async def qrcode_page(request: Request): return render_template("qrcode.html", {"request": request})

@app.get("/api/eventos/listar")
async def listar_eventos_api():
    db = SessionLocal(); eventos = db.query(Evento).order_by(Evento.data).all()
    resultado = [{"id":e.id,"nome":e.nome,"data":str(e.data) if e.data else "","local":e.local,"tipo":e.tipo,"descricao":e.descricao} for e in eventos]
    db.close(); return JSONResponse(resultado)

@app.get("/api/kits/listar")
async def listar_kits_api():
    db = SessionLocal(); kits_list = db.query(Kit).order_by(Kit.nome).all()
    resultado = [{"id":k.id,"nome":k.nome,"descricao":k.descricao,"preco_venda":k.preco_venda,"margem_percentual":k.margem_percentual,"itens":[{"id":i.id,"nome":i.nome,"preco_custo":i.preco_custo,"quantidade":1} for i in k.itens]} for k in kits_list]
    db.close(); return JSONResponse(resultado)

@app.get("/static/manifest.json")
async def manifest(): return FileResponse("app/static/manifest.json", media_type="application/json")

@app.get("/api/fornecedores")
async def listar_fornecedores_api():
    db = SessionLocal(); fl = db.query(Fornecedor).order_by(Fornecedor.nome).all()
    resultado = [{"id":f.id,"nome":f.nome,"cnpj":f.cnpj,"contato":f.contato,"telefone":f.telefone,"email":f.email,"cep":f.cep,"logradouro":f.logradouro,"bairro":f.bairro,"cidade":f.cidade,"uf":f.uf,"observacoes":f.observacoes} for f in fl]
    db.close(); return JSONResponse(resultado)

@app.get("/api/clientes/listar")
async def listar_clientes_api():
    db = SessionLocal(); cl = db.query(Cliente).order_by(Cliente.nome).all()
    resultado = [{"id":c.id,"nome":c.nome,"telefone":c.telefone,"email":c.email,"cpf":c.cpf,"data_nascimento":str(c.data_nascimento) if c.data_nascimento else "","endereco":c.endereco,"total_compras":c.total_compras,"ultima_compra":str(c.ultima_compra) if c.ultima_compra else "","observacoes":c.observacoes} for c in cl]
    db.close(); return JSONResponse(resultado)
...
async def auditoria_html(request: Request): return render_template("auditoria.html", {"request": request})

@app.get("/fechamento", response_class=HTMLResponse)
async def fechamento_html(request: Request): return render_template("fechamento.html", {"request": request})